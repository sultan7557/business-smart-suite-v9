...
import { createInterestedPartyVersion, getInterestedPartyVersions, deleteInterestedPartyVersion } from "@/app/actions/interested-party-actions"
...
export default function VersionHistoryDialog({ 
  ...
  const loadVersions = async () => {
    setLoading(true)
    try {
      const result = await getInterestedPartyVersions(interestedPartyId)
      if (result.success) {
  ...
  const handleCreateVersion = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await createInterestedPartyVersion(interestedPartyId, formData)
  ...
  const handleDeleteVersion = async (versionId: string) => {
    ...
    try {
      const result = await deleteInterestedPartyVersion(versionId)
      if (result.success) {
        toast({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Version History - {interestedPartyName}</DialogTitle>
        </DialogHeader>
