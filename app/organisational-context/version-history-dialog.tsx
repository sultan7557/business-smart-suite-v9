...
import { createOrganizationalContextVersion, getOrganizationalContextVersions, deleteOrganizationalContextVersion } from "@/app/actions/organizational-context-actions"
...
interface VersionHistoryDialogProps {
  ...
  organizationalContextName: string
}
...
export default function VersionHistoryDialog({ 
  ...
  useEffect(() => {
    if (open && organizationalContextId) {
      loadVersions()
    }
  }, [open, organizationalContextId])

  const loadVersions = async () => {
    setLoading(true)
    try {
      const result = await getOrganizationalContextVersions(organizationalContextId)
      if (result.success) {
  ...
  const handleCreateVersion = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await createOrganizationalContextVersion(organizationalContextId, formData)
  ...
  const handleDeleteVersion = async (versionId: string) => {
    ...
    try {
      const result = await deleteOrganizationalContextVersion(versionId)
      if (result.success) {
        toast({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Version History - {organizationalContextName}</DialogTitle>
        </DialogHeader>
