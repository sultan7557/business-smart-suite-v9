...
import { getObjectiveSectionReviews, createObjectiveSectionReview, deleteObjectiveSectionReview } from "@/app/actions/objective-actions";
...
export default function PageReviewsDialog({ 
  ...
  const loadReviews = async () => {
    setLoading(true);
    try {
      const result = await getObjectiveSectionReviews();
      if (result.success) {
  ...
  const handleCreateReview = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await createObjectiveSectionReview({
        reviewerName: formData.get("reviewerName") as string,
        reviewDetails: formData.get("reviewDetails") as string,
        reviewDate: formData.get("reviewDate") ? new Date(formData.get("reviewDate") as string) : new Date(),
  ...
  const handleDeleteReview = async (reviewId: string) => {
    ...
    try {
      const result = await deleteObjectiveSectionReview(reviewId);
      if (result.success) {
        toast({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Page Reviews - Objectives</DialogTitle>
        </DialogHeader>
