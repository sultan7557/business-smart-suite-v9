...
import { getObjectiveSectionVersions, createObjectiveSectionVersion, deleteObjectiveSectionVersion } from "@/app/actions/objective-actions";
...
export default function VersionHistoryDialog({ 
  ...
  const loadVersions = async () => {
    setLoading(true);
    try {
      const result = await getObjectiveSectionVersions();
      if (result.success) {
  ...
  const handleCreateVersion = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await createObjectiveSectionVersion({
  ...
  const handleDeleteVersion = async (versionId: string) => {
    ...
    try {
      const result = await deleteObjectiveSectionVersion(versionId);
      if (result.success) {
        toast({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Version History - Objectives</DialogTitle>
        </DialogHeader>
