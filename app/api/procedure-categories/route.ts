...
export const GET = withAuth(async (request: NextRequest) => {
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch procedure categories" }, { status: 500 })
  }
})
...
export const POST = withAuth(async (request: NextRequest) => {
  try {
    ...
    const highestOrderCategory = await prisma.procedureCategory.findFirst({
      orderBy: { order: "desc" },
      select: { order: true },
    })
    ...
    return NextResponse.json({ error: "Failed to create procedure category" }, { status: 500 })
  }
}, "write")