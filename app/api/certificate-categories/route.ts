...
export const GET = withAuth(async (request: NextRequest) => {
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch certificate categories" }, { status: 500 })
  }
})
...
export const POST = withAuth(async (request: NextRequest) => {
  try {
    ...
    return NextResponse.json({ error: "Failed to create certificate category" }, { status: 500 })
  }
}, "write")