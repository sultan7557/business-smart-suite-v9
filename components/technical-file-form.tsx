...
interface TechnicalFileFormProps {
  technicalFile: any
  canEdit: boolean
}

export default function TechnicalFileForm({ technicalFile, canEdit }: TechnicalFileFormProps) {
  const router = useRouter()
  const [isEditing, setIsEditing] = useState(false)
  const [formData, setFormData] = useState({
    title: technicalFile.title,
    version: technicalFile.version,
    reviewDate: new Date(technicalFile.reviewDate).toISOString().split("T")[0],
    department: technicalFile.department,
    content: technicalFile.content || "",
    highlighted: technicalFile.highlighted || false,
    approved: technicalFile.approved || false,
  ...
  const handleSave = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`/api/technical-file/${technicalFile.id}`, {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        throw new Error("Failed to update Technical File")
      }

      toast({
        title: "Success",
        description: "Technical File updated successfully",
      })
      ...
      toast({
        title: "Error",
        description: "Failed to update Technical File",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
