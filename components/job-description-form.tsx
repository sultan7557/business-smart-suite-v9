...
export default function JobDescriptionForm({ jobDescription, canEdit }: JobDescriptionFormProps) {
  const router = useRouter()
  const [isEditing, setIsEditing] = useState(false)
  const [formData, setFormData] = useState({
    title: jobDescription.title,
    version: jobDescription.version,
    reviewDate: new Date(jobDescription.reviewDate).toISOString().split("T")[0],
    nextReviewDate: jobDescription.nextReviewDate ? new Date(jobDescription.nextReviewDate).toISOString().split("T")[0] : "",
    department: jobDescription.department,
    content: jobDescription.content || "",
    highlighted: jobDescription.highlighted || false,
    approved: jobDescription.approved || false,
    categoryId: jobDescription.categoryId
  })
  ...
  const handleSave = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`/api/job-description/${jobDescription.id}`, {
        method: "PUT",
        headers: {
