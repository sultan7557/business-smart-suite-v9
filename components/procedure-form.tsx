...
export default function ProcedureForm({ procedure, canEdit }: ProcedureFormProps) {
  const router = useRouter()
  const [isEditing, setIsEditing] = useState(false)
  const [formData, setFormData] = useState({
    title: procedure.title,
    version: procedure.version,
    issueDate: new Date(procedure.issueDate).toISOString().split("T")[0],
    location: procedure.location,
    content: procedure.content || "",
    highlighted: procedure.highlighted || false,
    approved: procedure.approved || false,
