...
interface COSHHFormProps {
  coshh: any
  canEdit: boolean
}

export default function COSHHForm({ coshh, canEdit }: COSHHFormProps) {
  const router = useRouter()
  const [isEditing, setIsEditing] = useState(false)
  const [formData, setFormData] = useState({
    title: coshh.title,
    version: coshh.version,
    reviewDate: new Date(coshh.reviewDate).toISOString().split("T")[0],
    department: coshh.department,
    content: coshh.content || "",
    highlighted: coshh.highlighted || false,
    approved: coshh.approved || false,
  ...
  const handleSave = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`/api/coshh/${coshh.id}`, {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        throw new Error("Failed to update COSHH")
      }

      toast({
        title: "Success",
        description: "COSHH updated successfully",
      })
      ...
      toast({
        title: "Error",
        description: "Failed to update COSHH",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
