...
export default function CorrectiveActionForm({ correctiveAction, canEdit }: CorrectiveActionFormProps) {
  const router = useRouter()
  const [isEditing, setIsEditing] = useState(false)
  const [formData, setFormData] = useState({
    title: correctiveAction.title,
    version: correctiveAction.version,
    issueDate: new Date(correctiveAction.issueDate).toISOString().split("T")[0],
    location: correctiveAction.location,
    content: correctiveAction.content || "",
    highlighted: correctiveAction.highlighted || false,
    approved: correctiveAction.approved || false,
  ...
  const handleSave = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`/api/corrective-actions/${correctiveAction.id}`, {
        method: "PUT",
        headers: {
